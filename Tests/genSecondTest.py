...
if __name__ == "__main__":
    ...
    testsfile = open('second_tests.test', 'w')
    conn = sqlite3.connect('GAL.db')
    first_results = conn.cursor()
    second_results = conn.cursor()

    query = 'SELECT MIN(ga_db), ga_id FROM genetico, geneticop WHERE ga_type = gap_id AND ga_kf >= 5 GROUP BY ga_type ORDER BY AVG(ga_db) ASC LIMIT 20'
    ...
    for first_result in first_results:
        query = 'SELECT '+params+' FROM genetico, geneticop WHERE ga_type = gap_id AND ga_kf >= 5 AND ga_id = ' + str(first_result[1])
        print query
